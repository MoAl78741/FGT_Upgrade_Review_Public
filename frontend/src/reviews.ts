...
export type Review = ReviewInput & {completed_at?: string | null; id: string; revision: number; created_at: string; updated_at: string; expires_at?: string; job_ids: string[]; decisions: Record<string, Decision>; checklist: CheckItem[]};
export type Finding = {reference?: {available?:boolean; kind: 'pdf' | 'web'; url: string; page?: number; precision: 'section' | 'document'; name?: string} | null; id: string; job_id: string; version: string; section: string; source: {content?: string; blocks?: RichBlock[]; Description?: string; description?: string; markdown?: string; category?: string; title?: string; 'Bug ID'?: string; 'Feature ID'?: string}};
