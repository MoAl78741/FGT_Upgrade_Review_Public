...
function FindingCard({finding, decision, save, busy, next}: {finding: Finding; decision?: Decision; save: (value: Decision) => Promise<unknown>; busy: boolean; next:()=>void}) {
  ...
  return <article className="border border-navy-600 rounded-lg p-4 space-y-3">
    ...
    {finding.reference && canSource ? <a className="inline-block text-sm text-brand-500 underline" href={finding.reference.url} target="_blank" rel="noopener noreferrer">{finding.reference.kind === 'pdf' ? finding.reference.page ? `Open source PDF · section starts on page ${finding.reference.page}` : 'Open source PDF · page not recorded' : 'Open original web section'}</a> : <p className="text-xs text-gray-500">Original page link not recorded for this source.</p>}
    {canSource&&finding.reference?.kind==='pdf'&&<button type="button" className="text-brand-500 underline" onClick={()=>setSourceOpen(true)}>Compare with source PDF</button>}
