...
The release pipeline blocks fixable HIGH/CRITICAL image vulnerabilities. Unfixed findings remain in the complete `image-vulnerability-inventory` JSON artifact (30-day retention); scanner errors still fail the job. This is a release policy, not a claim that unfixed vulnerabilities are safe. Private publisher corpus replays remain separate operator-run release checks described above; confidential PDFs are not uploaded to ordinary GitHub runners. Branch protection rules are not modified by these workflow files; repository administrators can make `regression / regression` and `security / tests` required checks using the names shown by GitHub.
